...
import {
  ...
  X, ChevronDown, Grid3X3, List,
} from "lucide-react";
import { MOCK_SALONS, CATEGORIES, CITIES } from "@/lib/mock-data";
  ...
  return (
    <div className="animate-fade-in-up" style={{ animationDelay: `${delay}ms` }}>
      <Link href={`/salonlar/${salon.slug}`}>
        <div className="group rounded-2xl border overflow-hidden card-hover h-full flex flex-col"
          ...
          <div className="relative h-52 overflow-hidden bg-[var(--cream-3)]">
            {salon.cover_image && !imgError ? (
              <>
                ...
                />
                <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(28,23,20,0.55) 0%, rgba(28,23,20,0.05) 55%, transparent 100%)" }} />
              </>
            ) : (
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-5xl font-light tracking-widest opacity-20" style={{ color: "var(--gold)", fontFamily: "var(--font-playfair)" }}>{initials}</span>
              </div>
            )}
            <div className="absolute top-3 left-3 flex gap-2 z-10">
              {salon.is_featured && (
                <span className="text-[9px] tracking-widest uppercase px-2.5 py-1 rounded-full font-medium" style={{ background: "var(--gold)", color: "#fff" }}>Öne Çıkan</span>
              )}
              {salon.is_verified && (
          ...
          <div className="p-5 flex flex-col flex-1">
            <h3 className="font-medium text-sm mb-1.5 leading-tight" style={{ color: "var(--ink)", fontFamily: "var(--font-playfair)" }}>{salon.name}</h3>
            <div className="flex items-center gap-1 mb-3">
              <MapPin size={11} style={{ color: "var(--gold)" }} />
              <span className="text-[11px]" style={{ color: "var(--muted)" }}>{salon.district}, {salon.city}</span>
            </div>
            <p className="text-[12px] leading-relaxed mb-4 line-clamp-2 flex-1" style={{ color: "var(--muted)" }}>{salon.description}</p>
            <div className="flex items-center justify-between mt-auto">
              <div className="flex flex-wrap gap-1.5">
                {salon.categories.slice(0, 2).map((cat: string) => (
                  <span key={cat} className="text-[9px] tracking-wide uppercase px-2 py-0.5 rounded-full" style={{ background: "var(--gold-pale)", color: "var(--gold-dim)" }}>{cat}</span>
                ))}
              </div>
...
function SalonCardList({ salon, delay = 0 }: { salon: Salon; delay?: number }) {
  ...
  return (
    <div className="animate-fade-in-up" style={{ animationDelay: `${delay}ms` }}>
      <Link href={`/salonlar/${salon.slug}`}>
        <div className="group flex gap-5 rounded-2xl border overflow-hidden p-4 card-hover"
          style={{ background: "var(--white)", borderColor: "var(--border-ink)", boxShadow: "var(--shadow-sm)" }}>
          <div className="relative w-32 sm:w-44 flex-shrink-0 rounded-xl overflow-hidden" style={{ minHeight: "110px", background: "var(--cream-3)" }}>
            {salon.cover_image && !imgError ? (
              <Image src={salon.cover_image} alt={salon.name} fill
                className="object-cover transition-transform duration-500 group-hover:scale-105"
                onError={() => setImgError(true)} sizes="176px"
              />
            ) : (
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-2xl font-light opacity-20" style={{ color: "var(--gold)" }}>{salon.name.slice(0, 2)}</span>
              </div>
            )}
          </div>
          <div className="flex-1 min-w-0 py-1">
            <div className="flex items-start justify-between gap-3 mb-1.5">
              <h3 className="font-medium text-sm leading-tight" style={{ color: "var(--ink)", fontFamily: "var(--font-playfair)" }}>{salon.name}</h3>
            ...
            <p className="text-[12px] leading-relaxed mb-3 line-clamp-2 hidden sm:block" style={{ color: "var(--muted)" }}>{salon.description}</p>
...
export default function SalonlarPage() {
  ...
  const filtered = useMemo(() => {
    let result = MOCK_SALONS.filter((s: Salon) => {
      ...
      const matchCity = city === "Tüm Şehirler" || s.city === city;
    ...
    else if (sort === "name") result = [...result].sort((a, b) => a.name.localeCompare(b.name, "tr"));
  ...
  const activeFiltersCount = [
    search,
    city !== "Tüm Şehirler" ? city : "",
    category !== "all" ? category : "",
  ].filter(Boolean).length;

  const clearFilters = () => { setSearch(""); setCity("Tüm Şehirler"); setCategory("all"); };
  ...
  return (
    <main style={{ background: "var(--cream)", minHeight: "100vh" }}>
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="animate-fade-in-up">
              <p className="text-[10px] tracking-[6px] uppercase mb-2" style={{ color: "var(--gold-light)" }}>Türkiye Geneli • 500+ Salon</p>
            ...
            <div className="animate-scale-in flex items-center gap-3 rounded-2xl px-4 py-3 flex-1 md:max-w-md"
      ...
      <div className="sticky top-0 z-40 border-b"
        style={{ background: "rgba(250,248,246,0.97)", backdropFilter: "blur(12px)", borderColor: "var(--border-ink)" }}>
          ...
          <div className="flex items-center gap-2 overflow-x-auto">
            ...
            {CATEGORIES.map((cat) => (
              <button key={cat.value}
                ...
                className="text-[10px] tracking-wide uppercase px-3.5 py-1.5 rounded-full font-medium whitespace-nowrap transition-all duration-200 flex-shrink-0"
                style={{
            ...
            <div className="relative flex-shrink-0">
              <select value={city} onChange={(e) => setCity(e.target.value)}
                ...
                style={{
                  background: city !== "Tüm Şehirler" ? "var(--gold-pale)" : "var(--cream-2)",
                  color: city !== "Tüm Şehirler" ? "var(--gold-dim)" : "var(--muted)",
                  border: `1.5px solid ${city !== "Tüm Şehirler" ? "var(--gold)" : "transparent"}`,
                }}
              >
            ...
            <div className="ml-auto flex items-center gap-2 flex-shrink-0">
              {activeFiltersCount > 0 && (
                <button onClick={clearFilters}
                  className="flex items-center gap-1.5 text-[10px] tracking-wide uppercase px-3 py-1.5 rounded-full font-medium"
      ...
      <section className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm" style={{ color: "var(--muted)" }}>
            ...
            {search && <span> — <span style={{ color: "var(--gold)" }}>&ldquo;{search}&rdquo;</span> için</span>}
          </p>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-sm mb-6" style={{ color: "var(--muted)" }}>Filtreleri değiştirmeyi deneyin</p>
            <button onClick={clearFilters} className="btn-gold rounded-xl">Filtreleri Temizle</button>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((salon: Salon, i: number) => (
              <SalonCardGrid key={salon.id} salon={salon} delay={i * 50} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col gap-4">
            {filtered.map((salon: Salon, i: number) => (
              <SalonCardList key={salon.id} salon={salon} delay={i * 50} />
            ))}
          </div>
        )}
      </section>
